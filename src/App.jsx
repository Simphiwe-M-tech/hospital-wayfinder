...
function ShellHeader({
  ...
  return (
    <header className="flex items-center justify-between gap-3 border-b border-line px-5 py-3.5">
      ...
      <p
        className="shrink-0 text-right text-[11px] leading-snug text-inksoft"
        aria-live="polite"
      >
        {currentNode
...
export default function App() {
  ...
  const speak = useCallback(
    (message, interrupt = true) => {
      if (!voiceSupported || !message) return
  ...
  function handleScan(code) {
    ...
    if (result === 'advance') {
      ...
      setNotice({
        text: `You are still at ${byId.get(id).name}. Scan the next checkpoint to continue.`,
        tone: 'info',
      })
    } else {
      const nextRoute = findRoute(
        map,
        id,
        destinationId,
        accessible,
      ...
      if (nextRoute) {
        setNotice({
          text: `Route updated from ${byId.get(id).name}. Follow the updated directions.`,
          tone: 'success',
        })
      }
    }
