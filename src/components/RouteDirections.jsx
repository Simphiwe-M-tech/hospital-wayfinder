...
function stepIcon(step) {
  ...
  if (
    text.startsWith('Take the lift') ||
    text.startsWith('Take the stairs')
  ) {
  ...
  if (text.startsWith('Head towards')) {
    return Compass
  }

  if (text.startsWith('Continue straight')) {
    return ArrowRight
  }

  if (text.startsWith('Turn right')) {
    return CornerUpRight
  }

  if (text.startsWith('Turn left')) {
    return CornerUpLeft
  }

  if (text.startsWith('Turn back')) {
    return RotateCcw
  }
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="screen-sub mt-1.5">
          ...
          and scan each checkpoint as you reach it.
        </p>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        {route && usesLift && (
          <p className="border-b border-line bg-teal-soft/40 px-4 py-2 text-[11px] leading-snug text-inksoft">
            Walking time only — lift waiting and
            ride time are not included
            (unverified).
          </p>
        )}
